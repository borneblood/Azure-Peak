...
<<<<<<< Updated upstream
	...
	spawn(15)
		switch(dirgeline)
			if(1)
				to_chat(owner, span_userdanger("Cold stone presses against you. Then it is gone. A young woman in the street clutches her throat, her breath failing her as she sinks to her knees. Yours returns, slow and full."))
			if(2)
				to_chat(owner, span_userdanger("A tired man pauses mid-step, brow furrowed as if confused. His hand trembles at his chest. You draw in air as his lungs refuse him."))
			if(3)
				to_chat(owner, span_userdanger("Salt stings your lungs. A sailor, rope in hand, loses his footing on wet deck. He does not rise again. Your chest fills cleanly."))
			if(4)
				to_chat(owner, span_userdanger("A kneeling girl whispers a prayer. Her voice thins, then vanishes. The ache leaves your bones."))
			if(5)
				to_chat(owner, span_userdanger("A farm boy coughs weakly at the edge of a bed, his small frame shaking. The coughing stops. Your sight sharpens."))
			if(6)
				to_chat(owner, span_userdanger("At a grave, an old woman exhales through trembling lips. Her shoulders fall still. You breathe in her place."))
			if(7)
				to_chat(owner, span_userdanger("A child cries in the dark, small hands reaching for someone unseen. The crying fades into nothing. Warmth spreads through your veins."))
			if(8)
				to_chat(owner, span_userdanger("There is a flash of steel in a narrow alley. A young man stiffens as the blade finds him. He collapses. Strength returns to your limbs."))
			if(9)
				to_chat(owner, span_userdanger("Among the trees, a hunter staggers, one hand pressed to his side. He sinks into the leaves, breath leaving him. Your hands grow steady."))
			if(10)
				to_chat(owner, span_userdanger("A priest lifts his head to finish a prayer. His lips part, but no sound follows. Your breath deepens, calm and steady."))
			if(11)
				to_chat(owner, span_userdanger("Boots march through mud. A soldier lags behind, his step faltering. He drops, face first. Your chest rises with ease."))
			if(12)
				to_chat(owner, span_userdanger("A dagger is driven forward by unseen hands. It finds a woman's ribs, not yours. The pain that should have been yours is gone."))
			if(13)
				to_chat(owner, span_userdanger("A cradle rocks in a dim room. Inside, a small infant lies too still. The motion slows, then stops. Your heartbeat steadies."))
			if(14)
				to_chat(owner, span_userdanger("Flames climb hungrily around a figure you cannot fully see. A man's silhouette twists, then falls silent. The heat leaves your skin."))
			if(15)
				to_chat(owner, span_userdanger("Chains scrape across stone. A prisoner slumps forward, his weight going slack. The chains settle. You can move again."))
			if(16)
				to_chat(owner, span_userdanger("A wounded knight stands alone, armor split and dark with blood. His knees give way at last. Your wounds close without trace."))
			if(17)
				to_chat(owner, span_userdanger("Blood seeps into the earth beneath a fallen girl, her fingers digging weakly into the soil. Yours no longer spills."))
			if(18)
				to_chat(owner, span_userdanger("A stranger gasps for air, eyes wide with fear. His chest heaves once, twice, then stills. Your breath continues."))
			if(19)
